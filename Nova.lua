--[[
	...
	Basado en la arquitectura original de Sirius
...
if NovaUI then NovaUI.Name = "NovaUI" end
...
if secureMode and not customAssetId then
	secureNotify("default_asset", "Secure Mode", "You are using the default NovaUI asset ID.")
end
...
do
	...
	if hasCustomAsset and hasFilesystem then
		...
		secureNotify("no_getcustomasset", "NovaUI", "Your executor does not support getcustomasset.")
	end
...
function NovaUILibrary:CreateWindow(Settings)
	...
	-- Configuración predeterminada de ShowText a 'Show NovaUI'
	...
	if (Settings.KeySystem) then
		...
		if not Passthrough then
			...
			KeyMain.Title.Text = Settings.KeySettings.Title or Settings.Name or "NovaUI"
...
function NovaUILibrary:LoadConfiguration()
	...
	if CEnabled then
		...
		elseif not success and not notified then
			...
			NovaUILibrary:Notify({Title = "NovaUI Configurations", Content = "We've encountered an issue loading your configuration correctly.", Image = 4384402990})
		end
	end
