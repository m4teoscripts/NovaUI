...
local NovaUI = useStudio and script.Parent:FindFirstChild('Rayfield') or game:GetObjects("rbxassetid://"..NovaUIAssetId)[1]
...
repeat
	...
	local toDestroy
	toDestroy, NovaUI = NovaUI, useStudio and script.Parent:FindFirstChild('Rayfield') or game:GetObjects("rbxassetid://"..NovaUIAssetId)[1]
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			...
			NovaUILibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "rayfieldOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
function NovaUILibrary:CreateWindow(Settings)
	...
	if Settings.LoadingTitle ~= "NovaUI Interface Suite" then
		LoadingFrame.Version.Text = "NovaUI UI"
	end
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					TweenService:Create(Button, TweenInfo.new(0.6, Enum.EasingStyle
